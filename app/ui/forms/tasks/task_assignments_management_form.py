...
class TaskAssignmentsManagementForm(QDialog):
    def __init__(self, task_id: int, parent=None):
        super().__init__()
        self.setWindowTitle("Управление участниками задачи")
        ...
        # Левая таблица: Участники задачи
        self.participants_table = QTableWidget()
        self.participants_table.setColumnCount(5)  # Уменьшили количество колонок
    ...
    def get_participants(self) -> list[UserResponse]:
        """Получает список участников задачи"""
        try:
    ...
    def get_available_users(self) -> list[UserResponse]:
        """Возвращает список пользователей, которые ещё не участвуют в задаче"""
    ...
    def handle_button_click(self, user: UserResponse, is_participant: bool):
        ...
        if is_participant:
            try:
                ...
                self.client.managers.tasks.remove_task_assignment(task_id=self.task_id, user_id=user.id)
            except Exception as e:
                QMessageBox.critical(self, "Ошибка удаления", f"Попробуйте снова или повторите позже")
        else:
            try:
                self.used_ids.add(user.id)
                self.client.managers.tasks.add_task_assignments(task_id=self.task_id, user_id=user.id)
            except Exception as e:
                QMessageBox.critical(self, "Ошибка добавления", f"Попробуйте снова или повторите позже")
